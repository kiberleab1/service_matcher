...
const USER_TYPES = config.get("Users.UserTypes");

async function registerTalent(req, res) {
  try {
      ...
      joi
        .object({
          ...
          area_of_expertise: joi
            ...
            .required(),
          prefered_working_locations: joi
            .array()
            .items(
              joi.object({
    ...
    const {
      ...
      accunt_status,
      password,
    } = mustValidate(validateSchema(), req.body);
    // hash password here
    ...
    if (error["detail"]) {
      ...
      res
        ...
        .send("Failed to complete registration, Please check form data");
    }
  }
}
...
async function updateProfile(req, res) {
  try {
    ...
    if (error["detail"]) {
      ...
      res
        .status(HTTP_CODES.BAD_REQUEST)
        .send("Failed to complete profile update");
    }
  }
}
  ...
  try {
    ...
    const users = await model.UserProfile.findAndCountAll({
      where: Sequelize.literal(
        `"category"::jsonb @> '[{"title": "${req.body.category}"}]'`
      ),
    });
    ...
    if (users && users["count"] > 0) {
      ...
      const users = await model.UserProfile.findAndCountAll({
        where: Sequelize.literal(
          `"area_of_expertise"::jsonb @> '[{"title": "${req.body.category}"}]'`
        ),
      });
      res.status(HTTP_CODES.OK).json(users);
    }
    ...
    if (error["detail"]) {
      ...
      res
        .status(HTTP_CODES.BAD_REQUEST)
        .send("Failed to fetch users by category");
    }
  }
}
    ...
    const users = await model.UserProfile.findAndCountAll({
      where: {
        availability: {
          ...
          [Sequelize.Op.and]: [
            Sequelize.literal(
              `EXTRACT(HOUR FROM CURRENT_TIMESTAMP) BETWEEN (SELECT (elem->>'start_hour')::integer FROM jsonb_array_elements("availability") AS elem WHERE elem ? 'start_hour')::integer AND (SELECT (elem->>'end_hour')::integer FROM jsonb_array_elements("availability") AS elem WHERE elem ? 'end_hour')::integer`
            ),
          ],
        },
      },
...
async function registerUser(req, res) {
  try {
    ...
    const { phoneNumber, firstName, lastName } = mustValidate(
      validateSchema(),
      req.body
    );
    const newUser = await model.UserProfile.create({
      ...
      firstName,
      lastName,
      userType: USER_TYPES.SimpleUser,
    });
    res.status(HTTP_CODES.CREATED).send(newUser);
  } catch (e) {
    console.log(e);
...
exports.registerTalent = registerTalent;
